...
class Application:
  def __init__(self, file, address:str, directory:Directory, readers:[DocumentReader], extractor:InformationExtractor, output:Output, search: FuzzySearch):
    self.file = file
    self.output = output
    self.search = search
  ...
  def run(self):
    ...
    csv = self.output.generate_csv(self.file, office_no, process_no, subject, originator, addresses[['Endereço', 'Bairro', 'Município']])
